 ...
 * net to 16
 */
static uint16_t myHtons(uint16_t netShort)
{
	uint16_t retValue;
...
void Init_FT3_Test(unsigned char FT3_heap_id)
{
	 ...
	 */
	pBF609_FPGA_FT3 = heap_malloc(FT3_heap_id, sizeof(FT3_TEST_DATA)+3);
	if(pBF609_FPGA_FT3 == NULL)
	{
		printf("FT3 send buffer malloc failed !\n");
	}
	...
	for(int i = 0; i < sizeof(FT3_TEST_DATA); i++)
		((uint8_t *)pBF609_FPGA_FT3)[i] = 0xA5;
